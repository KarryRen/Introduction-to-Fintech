...
""" Split Training and Prediction code. (Regression) for single stock one by one. """
...
def ss_pred_model(stock_file_name: str, root_save_path: str):
    ...
    with torch.no_grad():
        for batch_data in tqdm(test_loader):
            ...
            # doc the result in one iter
            now_step = last_step + preds.shape[0]
            preds_one_stock[last_step:now_step] = preds[:, 0].detach()
            labels_one_stock[last_step:now_step] = labels[:, 0].detach()
            last_step = now_step
