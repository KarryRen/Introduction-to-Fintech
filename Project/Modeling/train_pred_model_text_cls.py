# -*- coding: utf-8 -*-
# @Time    : 2024/11/4 00:09
# @Author  : Karry Ren

""" Train and pred model for text data using classification model. """
...
from factor_dataset import TextFactoDataset
...
def train_valid_text_model(root_save_path: str) -> None:
    ...
    train_dataset = TextFactoDataset(file_path=config.TEXT_FILE_PATH, label_type=config.TEXT_LABEL_TYPE, feature_type=config.TEXT_FEATURE_TYPE,
                                     data_type="train")
    train_loader = data.DataLoader(dataset=train_dataset, batch_size=config.BATCH_SIZE, shuffle=True)  # the train dataloader
    valid_dataset = TextFactoDataset(file_path=config.TEXT_FILE_PATH, label_type=config.TEXT_LABEL_TYPE, feature_type=config.TEXT_FEATURE_TYPE,
                                     data_type="valid")
    ...
    if config.MODEL == "MLP":
        model = MLP_Net(input_size=config.TEXT_INPUT_DIM, device=device)
    elif config.MODEL == "Big_MLP":
        model = Big_MLP_Net(input_size=config.TEXT_INPUT_DIM, device=device)
    elif config.MODEL == "Conv":
        model = Conv_Net(device=device)
    elif config.MODEL == "GRU":
        model = GRU_Net(input_size=config.TEXT_INPUT_DIM, device=device)
    elif config.MODEL == "LSTM":
        model = LSTM_Net(input_size=config.TEXT_INPUT_DIM, device=device)
    elif config.MODEL == "Transformer":
        model = Transformer_Net(d_feat=config.TEXT_INPUT_DIM, device=device)
    else:
    ...
    # save the metric
    pd.DataFrame(epoch_metric)[int(0.95 * config.EPOCHS + 1):].to_csv(f"{os_model_save_path}/model_metric.csv")
...
def pred_text_model(root_save_path: str) -> None:
    ...
    # ---- Make the dataset and dataloader ---- #
    test_dataset = TextFactoDataset(file_path=config.TEXT_FILE_PATH, label_type=config.TEXT_LABEL_TYPE, feature_type=config.TEXT_FEATURE_TYPE,
                                    data_type="test")
...
if __name__ == "__main__":
    ...
    # build up the PATH
    SAVE_PATH = f"exp_text/rs_{config.RANDOM_SEED}"
    ...
    train_valid_text_model(root_save_path=SAVE_PATH)

    # ---- Step 2. Pred model ---- #
    pred_text_model(root_save_path=SAVE_PATH)
