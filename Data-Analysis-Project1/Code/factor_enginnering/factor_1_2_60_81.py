...
df1 = pd.read_csv('./data/资产负债表.csv', usecols=['证券代码', '报表类型', '统计截止日期', '现金及存放中央银行款项', '流动资产合计', '资产总计', '短期借款', '应交税费', '流动负债合计'], low_memory=False)
...
df3 = pd.read_csv('./data/现金流量表(间接法).csv', usecols=['证券代码', '报表类型', '统计截止日期', '固定资产折旧、油气资产折耗、生产性生物资产折旧', '无形资产摊销', '长期待摊费用摊销'], low_memory=False)
...
df5['acc'] = ((df5['delta_流动资产合计'] - df5['delta_现金及存放中央银行款项']) - (df5['delta_流动负债合计'] - df5['delta_短期借款'] - df5['delta_应交税费']) - df5['折旧摊销'])/df5['资产总计'].replace(0, np.nan)
...
# df6 = df5[['证券代码', '统计截止日期', 'acc', 'absacc', 'pctacc', 'stdacc']]
# df6.to_csv('./factor/factor_1_2_60_81.csv', index=False, encoding='utf-8-sig')
