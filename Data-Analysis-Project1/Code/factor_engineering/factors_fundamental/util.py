...
def diff(df, col=None, freq='季'):
    ...
    if freq == '季' and not (set(df['统计截止日期'].apply(lambda x: x[5:])) == set(['03-31', '06-30', '09-30', '12-31'])):
        raise Exception('季度频率的会计期间格式错误')
    if freq == '半年' and not (set(df['统计截止日期'].apply(lambda x: x[5:])) == set(['06-30', '12-31'])):
        raise Exception('半年度频率的会计期间格式错误')
    if col is None:
        col = list(set(df.columns) - set(('证券代码', '统计截止日期')))
